...
def parse_xmp_data(image_path: str) -> dict:
    """
    Parses the XMP metadata from an image using Pillow and built-in XML parser.
    This avoids the need for external executables like exiftool.
    
    Returns:
        A dictionary with found gimbal data (Yaw, Pitch, Roll).
    """
    xmp_gimbal_data = {}
    raw_xmp = None

    # --- Step 1: Extract raw XMP string using Pillow ---
    try:
        with Image.open(image_path) as pil_img:
            if 'xmp' in pil_img.info:
                raw_xmp = pil_img.info['xmp']
            # Some images store it in a different tag
            elif 'XML:com.adobe.xmp' in pil_img.info:
                raw_xmp = pil_img.info['XML:com.adobe.xmp']
        
        if raw_xmp:
            print("[DEBUG] Successfully extracted raw XMP string.")
        else:
            print("[DEBUG] No raw XMP string found by PIL.")
            return {}

    except Exception as e:
        print(f"Error reading image with PIL for XMP: {e}")
        return {}

    # --- Step 2: Parse the XMP string with XML parser ---
    if raw_xmp:
        try:
            # Clean the raw XMP data (it can have null terminators or be bytes)
            if isinstance(raw_xmp, bytes):
                raw_xmp = raw_xmp.decode('utf-8', 'ignore')
            ...
            # Remove null bytes which can break XML parser
            raw_xmp = raw_xmp.replace('\x00', '')

            root = ET.fromstring(raw_xmp)
            
            # Define common namespaces (this is critical)
            namespaces = {
                'rdf': 'http://www.w3.org/1999/02/22-rdf-syntax-ns#',
                'drone-dji': 'http://www.dji.com/drone-dji/1.0/',  # For DJI
                # Add other namespaces here if your drone is different
            }

            # --- Search for gimbal data ---
            # XMP data is often in an <rdf:Description> tag
            rdf_desc_path = './/rdf:Description'
            description_tags = root.findall(rdf_desc_path, namespaces)
            
            # We need to use the full namespace URL in curly braces
            yaw_key = '{http://www.dji.com/drone-dji/1.0/}GimbalYawDegree'
            pitch_key = '{http://www.dji.com/drone-dji/1.0/}GimbalPitchDegree'
            roll_key = '{http://www.dji.com/drone-dji/1.0/}GimbalRollDegree'

            for desc in description_tags:
                # Check if the keys exist as attributes in this tag
                if yaw_key in desc.attrib:
                    xmp_gimbal_data['Gimbal Yaw Degree'] = desc.attrib[yaw_key]
                if pitch_key in desc.attrib:
                    xmp_gimbal_data['Gimbal Pitch Degree'] = desc.attrib[pitch_key]
                if roll_key in desc.attrib:
                    xmp_gimbal_data['Gimbal Roll Degree'] = desc.attrib[roll_key]
            
            # Debug prints
            if 'Gimbal Yaw Degree' in xmp_gimbal_data:
                print(f"[DEBUG] Found Yaw (XMP/PIL): {xmp_gimbal_data['Gimbal Yaw Degree']}")
            if 'Gimbal Pitch Degree' in xmp_gimbal_data:
                print(f"[DEBUG] Found Pitch (XMP/PIL): {xmp_gimbal_data['Gimbal Pitch Degree']}")
            if 'Gimbal Roll Degree' in xmp_gimbal_data:
                print(f"[DEBUG] Found Roll (XMP/PIL): {xmp_gimbal_data['Gimbal Roll Degree']}")

        except ET.ParseError as e:
            print(f"Error parsing XMP XML: {e}")
        except Exception as e:
            print(f"An unexpected error occurred during XMP parsing: {e}")

    return xmp_gimbal_data


def parse_image(image_path: str) -> dict:
    """
    Extract metadata from an image using exif library (basic) and
    our custom XMP parser (for critical gimbal data).
    """
    parsed_data = {}
    print(f"\n--- Parsing: {os.path.basename(image_path)} ---")

    # --------------------------------------------------------------------
    # Step 1: Use our custom XMP parser
    # --------------------------------------------------------------------
    xmp_gimbal_data = parse_xmp_data(image_path)

    # --------------------------------------------------------------------
    # Step 2: Use 'exif' library for basic data
    # --------------------------------------------------------------------
    try:
        with open(image_path, 'rb') as image_file:
            ...
            if all(hasattr(img, attr) for attr in ['gps_latitude', 
                ...
                print("[DEBUG] Could not find standard GPS tags in 'exif' library.")
            ...
            if hasattr(img, 'gps_altitude'):
                ...
                parsed_data["Relative Altitude"] = str(altitude) # Default
            
            # Field of view - default
            parsed_data["Field Of View"] = "75.0"

    except Exception as e:
        print(f"Error reading basic EXIF data from {image_path}: {str(e)}")
        # If we failed here and also with XMP, we can't proceed
        if not xmp_gimbal_data:
            return None

    # --------------------------------------------------------------------
    # Step 3: OVERWRITE defaults with accurate XMP data (if found)
    # --------------------------------------------------------------------

    # --- Gimbal Yaw (THE CRITICAL FIX) ---
    if 'Gimbal Yaw Degree' in xmp_gimbal_data:
        parsed_data["Gimbal Yaw Degree"] = str(xmp_gimbal_data['Gimbal Yaw Degree'])
    else:
        parsed_data["Gimbal Yaw Degree"] = "0.0"
        print("⚠️ [WARNING] Could not find Gimbal Yaw XMP tag. Defaulting to 0.0.")

    # --- Gimbal Pitch ---
    if 'Gimbal Pitch Degree' in xmp_gimbal_data:
        parsed_data["Gimbal Pitch Degree"] = str(xmp_gimbal_data['Gimbal Pitch Degree'])
    else:
        parsed_data["Gimbal Pitch Degree"] = "-90.0"
        print("⚠️ [WARNING] Could not find Gimbal Pitch XMP tag. Defaulting to -90.0 (nadir).")

    # --- Gimbal Roll ---
    if 'Gimbal Roll Degree' in xmp_gimbal_data:
        parsed_data["Gimbal Roll Degree"] = str(xmp_gimbal_data['Gimbal Roll Degree'])
    else:
        parsed_data["Gimbal Roll Degree"] = "0.0"
        print("⚠️ [WARNING] Could not find Gimbal Roll XMP tag. Defaulting to 0.0.")

    # --- Check for missing essentials ---
    if "GPS Latitude" not in parsed_data or "Exif Image Width" not in parsed_data:
        print(f"❌ [ERROR] Missing critical data (GPS or Dimensions) for {image_path}. Skipping.")
        return None
...
def create_geotiff(parsed_data, source_jpg_path, output_tiff_path, compress=False, quality=100):
    """
    Creates a georeferenced TIFF from a JPG using its parsed EXIF/XMP metadata.
    Optionally draws graphics (circles) for points in gdf *after* creation.
    """
    ...
    try:
        ...
        if lat_dd is None or lon_dd is None:
             raise ValueError("Failed to parse GPS coordinates.")
        
        altitude_m = float(parsed_data['Absolute Altitude']) 
        
        # Camera orientation (gimbal) - NOW ACCURATE!
        ...
        print(f"❌ [ERROR] Missing or invalid essential metadata: {e}")
        return
    ...
    # --- Step E: Write the georeferenced TIFF file ---
    try:
        with rasterio.open(source_jpg_path) as src:
            ...
            # Write the new GeoTIFF file (without graphics first)
            with rasterio.open(output_tiff_path, 'w', **profile) as dst:
                dst.write(src_data)

        print(f"✅ Successfully created georeferenced file: {output_tiff_path}")

        # --- Step F: (FIXED) Draw graphics *after* file is created ---
        if draw_graphic and gdf is not None:
            print("Drawing graphics on GeoTIFF...")
            try:
                # Re-open the file in read/write mode ('r+')
                with rasterio.open(output_tiff_path, 'r+') as dst:
                    # Convert GeoTIFF data to PIL Image
                    arr = dst.read()
                    im = Image.fromarray(np.moveaxis(arr[:3], 0, -1))  # (H, W, C)
                    draw = ImageDraw.Draw(im)

                    circle_polys = []
                    
                    # Convert GDF to the image's CRS *once*
                    gdf_in_crs = gdf.to_crs(dst.crs)
                    
                    # Get image bounds
                    img_bounds_poly = Polygon.from_bounds(*dst.bounds)

                    for idx, row in gdf_in_crs.iterrows():
                        point = row.geometry
                        
                        # Check if point is inside the image bounds
                        if point.within(img_bounds_poly):
                            # Get the *correct* pixel coordinates using the transform
                            px, py = dst.index(point.x, point.y)
                            
                            # Create a Shapely circle in pixel coordinates
                            circle = Point(px, py).buffer(graphic_radius, resolution=32)
                    ...
                    if circle_polys:
                        ...
                        arr2 = np.array(im)  # (H, W, C)
                        # Convert back to (C, H, W) for rasterio
                        arr_out = np.moveaxis(arr2, -1, 0)
                        
                        # Write the modified data back to the GeoTIFF
                        dst.write(arr_out)
                        print("✅ Successfully drew graphics.")
...
def image_contains_points(parsed_data: dict, gdf: gpd.GeoDataFrame) -> bool:
    ...
    try:
        ...
        if lat_dd is None or lon_dd is None:
            print("Skipping footprint check due to unparsed GPS.")
            return False
        ...
        # Create a GeoSeries for the footprint to do CRS operations
        footprint_gs = gpd.GeoSeries([image_footprint], crs="EPSG:4326")
        
        # Spatially join the points with the footprint polygon
        # This is much faster than iterating
        points_in_footprint = gpd.sjoin(gdf, gpd.GeoDataFrame(geometry=footprint_gs), how='inner', predicate='within')

        return not points_in_footprint.empty
...
def process_excel_to_geojson(src_path: str) -> str:
    ...
    # 4. Filter to just rows *after* the header
    metadata_df = metadata_df.iloc[header_row_index + 1:].reset_index(drop=True)

    # 5. Force columns to numeric, coercing errors to NaN
...
def main():
    ...
    if args.output_dir:
        georef_parent_dir = os.path.abspath(args.output_dir)
    elif args.recursive:
    ...
    for i_idx, image_info in enumerate(image_files, 1):
        ...
        # Check for final output file (e.g. .jpg) not just intermediate .tif
        final_output_path = os.path.splitext(output_path)[0] + f".{args.format}"
        if os.path.exists(final_output_path):
            print(f"File {final_output_path} already exists. Skipping...")
            continue
        ...
        try:
            ...
            create_geotiff(
                ...
                image_path,
                output_path,
                compress=(args.quality < 100),
                quality=args.quality
            )

            # Convert to desired format if not TIF
            if args.format != 'tif':
                final_path = convert_to_image_format(output_path, args.format, args.quality)
                if final_path != output_path:
                    print(f"✅ Converted to {args.format}: {os.path.basename(final_path)}")
                else:
                    print(f"⚠️ Conversion to {args.format} failed. Kept TIFF.")
    ...
    print(f"Output saved to: {georef_parent_dir}")
    print("\nProcessing complete!")
